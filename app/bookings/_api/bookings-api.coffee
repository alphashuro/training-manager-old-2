create = ( booking ) ->
  Meteor.call 'create/booking', booking, ( error, bookingId ) ->
    if error
...
remove = ( _id ) ->
  Meteor.call 'remove/booking', _id, ( error ) ->
    if error
      App.utils.notify.error error.reason
    else
...
addStudent = ( { _id, studentId } ) ->
  Meteor.call 'addStudent/booking', { _id, studentId } , ( error ) ->
    if error
      App.utils.notify.error error.reason

removeStudent = ( { _id, studentId } ) ->
  Meteor.call 'removeStudent/booking', { _id, studentId } , ( error ) ->
    if error
      App.utils.notify.error error.reason
